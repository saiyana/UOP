...
def getSelected(topics,summaries):
    selectedTopics = raw_input('Enter the topics that are to be selected: ')
    selectedTopics = [int(x) for x in selectedTopics.split(',')]
    selectedSummaries = []
    for i in selectedTopics:
        selectedSummaries.append(summaries[i])
    return [selectedTopics,selectedSummaries]
...
def start(tweeters, tweets):
    ...
    [selectedTopicsNMF,selectedSummaryNMF]=getSelected(topicsListNMF,summaryListNMF)
    [selectedTopicsLDA, selectedSummaryLDA]=getSelected(topicsListLDA, summaryListLDA)

    topicSummarization.start(selectedTopicsNMF,selectedSummaryNMF,selectedTopicsLDA,selectedSummaryLDA)
